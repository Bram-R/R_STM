...
This study adopted a structured approach to evaluate and optimise the performance of a commonly used state transition model (STM) code based on the DARTH framework in R. To this end, we systematically explored the baseline code and 20 different computational approaches to assess their impact on execution time, memory usage, and garbage collection (GC). The performance metrics were measured using the "bench" package (version `r packageVersion("bench")`),[@R-bench] which allowed for simultaneous assessment of these key parameters.

## Tested model

These different models allowed us to systematically evaluate the impact of various modifications on execution time, memory usage, and GC.

For this purpose, we implemented five distinct models (A to E), each with a unique focus on different aspects of computational optimisation. The objective was to test a variety of strategies, from data structure conversions to advanced coding techniques.
